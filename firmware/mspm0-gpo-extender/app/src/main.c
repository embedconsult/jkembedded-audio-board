...
static int pca9538_apply_output_latches(void)
{
	for (size_t i = 0; i < ARRAY_SIZE(mux_gpios); i++) {
		...
		bool is_input = (pca9538.config & BIT(i)) != 0U;
		bool level = (pca9538.output & BIT(i)) != 0U;
		int ret;
...
static int pca9538_apply_config(void)
{
	for (size_t i = 0; i < ARRAY_SIZE(mux_gpios); i++) {
		...
		bool level = (pca9538.output & BIT(i)) != 0U;
		gpio_flags_t flags =
			is_input ? GPIO_INPUT : (level ? GPIO_OUTPUT_ACTIVE : GPIO_OUTPUT_INACTIVE);
		int ret;
...
static void pca9538_write_reg(uint8_t reg, uint8_t value)
{
	...
	case PCA9538_REG_OUTPUT:
		...
		(void)pca9538_apply_output_latches();
		break;
	case PCA9538_REG_POLARITY:
